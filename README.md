# Hi, I'm Tanatswa Clayton Choga aka Gancholin 👋

**Telecommunications Engineer • Network Specialist • Web Developer**
Based in Zimbabwe  | Open to remote roles & relocation 🌍

I work on the **technical side of networks** — fiber, VoIP, cloud, and security —
and I also **build websites** for businesses and personal projects.

## 🔧 What I Do
- 🌐 Design & maintain **high-availability networks** (fiber, VoIP, cloud)
- 🔒 Secure infrastructure with **CCNA-level + cybersecurity** expertise
- 💻 Build **websites** — clean, fast, responsive
- ⚙️ Troubleshoot complex network issues and automate where it matters
- 🛠️ Former Network Engineer at **TelOne Zimbabwe**

## 🎓 Education & Certifications
**Diploma in Telecommunications Engineering**
TelOne Centre for Learning

**Certifications:**
- ✅ CCNA — Switching & Routing
- ✅ CCNA — Enterprise
- ✅ CCNA — Security
- ✅ EC-Council Cybersecurity

## 💻 Tech & Tools
**Networking:** Cisco, Fiber, VoIP, Cloud Infrastructure
**Web:** HTML, CSS, JavaScript
**Other:** Python, Automation

## 🚀 Currently
- Building websites for clients & personal projects
- Open to new opportunities — remote or relocation

## 🌍 Let's Connect
📫 **Email:** tchoga00@gmail.com
💼 **LinkedIn:** [tanatswa-clayton-choga](https://www.linkedin.com/in/tanatswa-clayton-choga-62b659340)
🌐 **Portfolio:** [tchoga.netlify.app](https://tchoga.netlify.app)
